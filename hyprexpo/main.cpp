...
typedef void (*origRenderWorkspace)(void*, PHLMONITOR, PHLWORKSPACE, timespec*,
                                    const CBox&);
...
static void hkAddDamageA(void* thisptr, const CBox& box) {
    ...
    if (!g_pOverview || g_pOverview->pMonitor != PMONITOR->m_self ||
        g_pOverview->blockDamageReporting) {
...
static SDispatchResult onExpoDispatcher(std::string arg) {
    ...
    if (index != -1) {
        if (g_pOverview) {
            g_pOverview->selectWorkspaceIndex(index);
            g_pOverview->close();
        }
        return {};
    }
...
static void failNotif(const std::string& reason) {
    HyprlandAPI::addNotification(
        PHANDLE, "[hyprexpo] Failure in initialization: " + reason,
        CHyprColor{1.0, 0.2, 0.2, 1.0}, 5000);
}

static Hyprlang::CParseResult expoGestureKeyword(const char* LHS,
                                                 const char* RHS) {
        ...
        if (data[startDataIdx].starts_with("mod:")) {
            ...
            try {
                deltaScale = std::clamp(
                    std::stof(std::string{data[startDataIdx].substr(6)}), 0.1F,
                    10.F);
                startDataIdx++;
                continue;
            } catch (...) {
                result.setError(
                    std::format("Invalid delta scale: {}",
    ...
    if (data[startDataIdx] == "expo")
        resultFromGesture = g_pTrackpadGestures->addGesture(
            makeUnique<CExpoGesture>(), fingerCount, direction, modMask,
            deltaScale);
...
APICALL EXPORT PLUGIN_DESCRIPTION_INFO PLUGIN_INIT(HANDLE handle) {
    ...
    if (HASH != CLIENT_HASH) {
        failNotif("Version mismatch (headers ver is not equal to running "
                  "hyprland ver)");
    ...
    if (FNS.empty()) {
        ...
        throw std::runtime_error(
            "[he] No fns for hook addDamageEPK15pixman_region32");
    }
    ...
    HyprlandAPI::addConfigValue(PHANDLE, "plugin:hyprexpo:gap_size",
                                Hyprlang::INT{5});
    HyprlandAPI::addConfigValue(PHANDLE, "plugin:hyprexpo:bg_col",
                                Hyprlang::INT{0xFF111111});
    HyprlandAPI::addConfigValue(PHANDLE, "plugin:hyprexpo:workspace_method",
                                Hyprlang::STRING{"center current"});
    HyprlandAPI::addConfigValue(PHANDLE, "plugin:hyprexpo:skip_empty",
                                Hyprlang::INT{0});

    HyprlandAPI::addConfigValue(PHANDLE, "plugin:hyprexpo:gesture_distance",
                                Hyprlang::INT{200});
